...
function calcInvoice() {
  ...
  document.querySelectorAll('#i-table-body tr').forEach(tr => {
    ...
    if (priceCell && totalCell) {
      const v = parseFloat(priceCell.textContent) || 0;
      sub += v;
      totalCell.textContent = v > 0 ? v.toFixed(2) + '$' : '';
    }
  });
...
function renderWorkers() {
  ...
  tbody.innerHTML = '';
  let totalDays = 0, totalGross = 0, totalRemain = 0;
  workers.forEach((w, i) => {
    const gross   = (w.rate || 0) * (w.days || 0) + (w.overtime || 0);
    const already = w.already || 0;
  ...
  document.getElementById('ws-total').textContent = '$' + totalGross.toFixed(2);
  document.getElementById('ws-avg').textContent   = '$' + totalRemain.toFixed(2);
}
...
function openWModal(idx = null) {
  ...
  document.getElementById('wf-overtime').value = w.overtime || '';
...
function applyWorker() {
  ...
  const w = {
    ...
    overtime: parseFloat(document.getElementById('wf-overtime').value) || 0,
...
function buildWorkerPrintDoc() {
  ...
  workers.forEach(w => {
    ...
    rows += `<tr>
      ...
      <td style="${TD}">${(w.overtime||0) > 0 ? (w.overtime).toFixed(2)+'$' : '0$'}</td>
      <td style="${TD}">${gross.toFixed(2)}$</td>
      <td style="${TD}">${already > 0 ? already.toFixed(2)+'$' : '0$'}</td>
      <td style="${TD};font-weight:700;">${remain.toFixed(2)}$</td>
    </tr>`;
  });
  if (!rows) rows = `<tr><td style="${TD}"> </td><td style="${TD}"> </td><td style="${TD}">0$</td><td style="${TD}"> </td><td style="${TD}"> </td><td style="${TD}"> </td></tr>`;
  ...
  return `<div id="worker-print-doc" style="background:#fff;width:794px;padding:40px 36px 60px;font-family:'DM Sans',sans-serif;font-size:12px;color:#1C1C1E;">
    ...
    <table style="width:100%;border-collapse:collapse;border:1.5px solid #2c3e6b;">
      <thead><tr>
        ...
        <th style="${TH};color:#27ae60;background:#fff;width:13%;">ចំនួនប្រាក់គិតថ្ងៃ</th>
        <th style="${TH};color:#2980b9;background:#fff;width:15%;">ថែមម៉ោង ( 0 ម៉ោង )</th>
        <th style="${TH};color:#c0392b;background:#fff;width:17%;">ចំនួនប្រាក់ដែលត្រូវបើក</th>
        <th style="${TH};color:#27ae60;background:#fff;width:17%;">ចំនួនប្រាក់ដែលបានសម្រួលមួស</th>
        <th style="${TH};color:#8e44ad;background:#fff;width:23%;">ចំនួនប្រាក់បើកសម្រាប់ថ្ងៃ<br/><span style="font-size:9.5px;">${dateFrom} ដល់ ${dateTo}</span></th>
      </tr></thead>
      <tbody>${rows}</tbody>
    </table>
...
async function saveWorkerPDF() {
  ...
  async function preloadSigB64() {
    return new Promise(resolve => {
      ...
      img.onload = () => { try { const c = document.createElement('canvas'); c.width = img.naturalWidth; c.height = img.naturalHeight; c.getContext('2d').drawImage(img,0,0); resolve(c.toDataURL('image/png')); } catch(e) { resolve(''); } };
      img.onerror = () => resolve('');
  ...
  if (sigB64) { const si = wrapper.querySelector('img'); if (si) si.src = sigB64; }
  document.body.appendChild(wrapper);
  try {
    await document.fonts.ready;
    await new Promise(r => setTimeout(r, 400));
    const canvas = await html2canvas(wrapper.firstElementChild, { scale:2, useCORS:true, allowTaint:true, logging:false, backgroundColor:'#ffffff' });
    const { jsPDF } = window.jspdf;
    const pdf = new jsPDF({ unit:'mm', format:'a4', orientation:'portrait' });
    const pageW = pdf.internal.pageSize.getWidth(), pageH = pdf.internal.pageSize.getHeight();
    const margin = 5, printW = pageW - margin*2;
    const printH = canvas.height * (printW / canvas.width);
    if (printH <= pageH - margin*2) { pdf.addImage(canvas,'PNG',margin,margin,printW,printH,'','FAST'); }
    else {
      const mmPerPx = printW/canvas.width, sliceH = Math.floor((pageH-margin*2)/mmPerPx);
      let y = 0;
      while (y < canvas.height) { const s = document.createElement('canvas'); s.width=canvas.width; s.height=Math.min(sliceH,canvas.height-y); s.getContext('2d').drawImage(canvas,0,-y); pdf.addImage(s,'PNG',margin,margin,printW,s.height*mmPerPx,'','FAST'); y+=sliceH; if(y<canvas.height) pdf.addPage(); }
    }
    pdf.save('Worker-Spend.pdf');
  } finally { document.body.removeChild(wrapper); btn.innerHTML = origHTML; btn.disabled = false; }
}
...
function printWorker() {
  ...
  div.id = 'worker-print-area'; div.className = 'worker-print-only';
...
function addMaoRow() {
  const tbody = document.getElementById('mao-table-body');
  const num = tbody.querySelectorAll('tr').length + 1;
  const tr = document.createElement('tr');
  tr.innerHTML = `
    <td class="tc-no" contenteditable="true">${num}</td>
    ...
    <td class="tc-size" contenteditable="true"></td>
    <td class="tc-price" contenteditable="true"></td>
    <td class="tc-price mao-total" contenteditable="true" oninput="calcMao()"></td>
    <td class="tc-act no-print"><button class="del-btn" onclick="deleteMaoRow(this)">✕</button></td>`;
  tbody.appendChild(tr);
  tr.querySelector('.tc-content').focus();
}

function deleteMaoRow(btn) { btn.closest('tr').remove(); renumberMao(); calcMao(); }
function renumberMao() {
  document.querySelectorAll('#mao-table-body tr').forEach((r,i) => {
    const c = r.querySelector('.tc-no'); if (c) c.textContent = i+1;
  });
}
...
function calcMao() {
  let sub = 0;
  document.querySelectorAll('#mao-table-body tr').forEach(tr => {
    const totalCell = tr.querySelector('.mao-total');
    if (totalCell) {
      sub += parseFloat(totalCell.textContent.replace(/[^0-9.]/g,'')) || 0;
    }
  });
  const depositText = document.getElementById('mao-deposit').textContent.replace(/[^0-9.]/g,'');
  const deposit = parseFloat(depositText) || 0;
  const total = sub - deposit;
  document.getElementById('mao-grand-total').textContent = total.toFixed(2) + '$';
}

function addMaoNote() {
  const list = document.getElementById('mao-notes-list');
  const n = list.querySelectorAll('.note').length + 1;
  const p = document.createElement('p');
  p.className = 'note khmer'; p.contentEditable = 'true'; p.textContent = `${n}. `;
  list.appendChild(p); p.focus();
...
function openMaoCustomerModal() {
  document.getElementById('mf-customer').value = document.getElementById('md-customer').textContent.trim();
  document.getElementById('mf-contact').value  = document.getElementById('md-contact').textContent.trim();
  document.getElementById('mf-address').value  = document.getElementById('md-address').textContent.trim();
  document.getElementById('mf-invno').value    = document.getElementById('md-invno').textContent.trim();
  document.getElementById('mf-date').value     = document.getElementById('md-date').textContent.trim();
  document.getElementById('maoCustomerModal').classList.add('open');
}
function closeMaoCustomerModal() { document.getElementById('maoCustomerModal').classList.remove('open'); }
function closeIfMaoOverlay(e) { if (e.target === document.getElementById('maoCustomerModal')) closeMaoCustomerModal(); }
function applyMaoCustomer() {
  document.getElementById('md-customer').textContent = document.getElementById('mf-customer').value;
  document.getElementById('md-contact').textContent  = document.getElementById('mf-contact').value;
  document.getElementById('md-address').textContent  = document.getElementById('mf-address').value;
  document.getElementById('md-invno').textContent    = document.getElementById('mf-invno').value;
  document.getElementById('md-date').textContent     = document.getElementById('mf-date').value;
  closeMaoCustomerModal();
}

async function saveMaoPDF() {
  await savePDF('mao-doc', 'mao-savebtn', 'Invoice Mao Ceiling', '.page-wrapper');
}
function saveMaoMobile() { mobilePrint(); }

/* ══════════════ KEYBOARD ══════════════ */
document.addEventListener('keydown', e => {
  if (e.key === 'Escape') {
    closeQCustomerModal(); closeICustomerModal(); closeWModal(); closeMaoCustomerModal();
  }
